...
namespace RTR {
    ...
    public:
        ...
        int validate(const uint32_t asn, const struct lrtr_ip_addr* prefix, const uint8_t mask_len) {

            enum pfxv_state result;
            
            int ret = rtr_mgr_validate(conf, asn, prefix, mask_len, &result);

            if (ret == PFX_SUCCESS) {
                if (result == BGP_PFXV_STATE_VALID) {
                    BOOST_LOG_TRIVIAL(info) << "MTR: valid";
                } else if (result == BGP_PFXV_STATE_INVALID) {
                    BOOST_LOG_TRIVIAL(info) << "MTR: invalid";
                } else if (result == BGP_PFXV_STATE_NOT_FOUND) {
                    BOOST_LOG_TRIVIAL(info) << "MTR: no certificate found";
                }
            } else {
                BOOST_LOG_TRIVIAL(info) << "MTR: call failed";
                return -1;
            }
